...
class Exit_interview extends MY_Controller
{
	...
	function view()
	{
		$conditions = [
			...
			'xer.exit_interview_status' => '0'
		];
	   
		$data['title'] = 'Exit Interviews';

		$filtered_conditions = $this->remove_empty_entries($conditions);

		$url = 'Exit_interview/view';
		$total_rows = $this->Exit_interview_model->get($filtered_conditions)->num_rows();
		$this->pagination_initializer($this->limit, $this->num_links, $total_rows, $url);
		$data['exit_interview'] = $this->Exit_interview_model->get($filtered_conditions)->result();

		$data['content'] = $this->load->view('exit_interview/index', $data, TRUE);
